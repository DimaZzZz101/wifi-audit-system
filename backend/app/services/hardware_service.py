"""Сбор информации о хосте для аудита Wi-Fi: USB, PCI, сетевые интерфейсы, файловые системы."""
import os
import re
import subprocess
...
# Таймаут для внешних команд (секунды)
_CMD_TIMEOUT = 10

_USB_WIFI_VENDORS = frozenset({
    "0bda", "148f", "0cf3", "2357", "0e8d", "2a70", "04e8", "0b05", "13b1", "050d", "07b8", "083a", "157e",
})
...
def _run(cmd: list[str]) -> list[str] | None:
    """Выполнить команду, вернуть строки stdout или None при ошибке."""
    try:
        r = subprocess.run(
            cmd,
            capture_output=True,
...
def _is_usb_wifi(usb_id: str, name: str) -> bool:
    try:
        ...
        if vendor in _USB_WIFI_VENDORS:
            return True
    except Exception:
        pass
    name_lower = name.lower()
    return any(s in name_lower for s in _WIFI_NAME_SUBSTRINGS)
...
def get_usb_devices() -> list[dict[str, Any]]:
    """
    Список USB-устройств (lsusb). Полезно для Wi-Fi: адаптеры по USB.
    В Docker без доступа к хосту вернёт пустой список или устройства контейнера.
    Добавляет wifi_capable.
    """
    out = _run(["lsusb"])
    if not out:
        return []
...
def get_pci_devices() -> list[dict[str, Any]]:
    """Список PCI-устройств (lspci). Добавляет wifi_capable. В Docker без /sys может быть пустым."""
    out = _run(["lspci", "-mm"])
    if not out:
        out = _run(["lspci"])
    if not out:
...
def get_network_interfaces() -> list[dict[str, Any]]:
    """
    Сетевые интерфейсы (в т.ч. wlan*, wlp*, wlx*). В Docker /sys - с хоста, ip - из контейнера.
    Используем /sys/class/net как primary. Беспроводные - по /sys/class/net/name/wireless.
    """
    net = Path("/sys/class/net")
    if not net.exists():
        return []

    # Primary: /sys/class/net - при монтировании /sys с хоста даёт интерфейсы хоста
...
def get_filesystem_usage() -> list[dict[str, Any]]:
    ...
    if not out:
        return []
    # Первая строка - заголовок (Filesystem Type Size Used Avail Use% Mounted on)
...
def get_hardware_summary(wifi_only: bool = False) -> dict[str, Any]:
    """Сводка по хосту: USB, PCI, интерфейсы, файловые системы. wifi_only - только Wi-Fi модули."""
    usb = get_usb_devices()
    pci = get_pci_devices()
    if wifi_only:
